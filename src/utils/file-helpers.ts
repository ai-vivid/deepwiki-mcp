 ...
 * // => "query"
 */
export function generateDescriptiveName(question?: string): string {
 ...
 */
export function getRepoFolder(repo: string): string {
  return repo.replace('/', '-');
}
 ...
 */
export async function ensureOutputStructure(repo: string, type: string): Promise<string> {
 ...
 * // => throws Error
 */
export function validateSaveLocation(saveLocation: string): string {
  ...
  if (!isWithinAllowedBase) {
    throw new Error(
      `Invalid save location: must be within one of the allowed directories: ${allowedBases.join(', ')}. Got: ${saveLocation}`
    );
  }
