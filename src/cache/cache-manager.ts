...
export class CacheManager {
  ...
  private getRepoFolder(repo: string): string {
    return repo.replace('/', '-');
  }

  private getCacheSubfolder(type: string): string {
  ...
  private async ensureCacheStructure(repo: string, type: string): Promise<string> {
  ...
  private getCacheFileName(type: string, identifier: string): string {
  ...
  async get(type: string, identifier: string, repo?: string): Promise<string | null> {
    try {
      if (repo) {
  ...
  async set(type: string, identifier: string, content: string, repo?: string): Promise<void> {
    try {
      if (repo) {
  ...
  async getJSON<T>(type: string, identifier: string, repo?: string): Promise<T | null> {
  ...
  async setJSON<T = unknown>(type: string, identifier: string, data: T, repo?: string): Promise<void> {
