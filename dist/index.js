...
import { ensureOutputStructure, validateSaveLocation } from "./utils/file-helpers.js";
...
function generateWikiDescription(action, chapters) {
    if (action === 'structure') {
        return 'structure';
    }
...
server.tool("wiki_parser", `A comprehensive tool for parsing and extracting content from DeepWiki documentation pages.
...
server.tool("wiki_question", `A powerful tool for asking questions about GitHub repositories using DeepWiki's AI analysis.
...
server.resource("available-commands", "deepwiki://commands", async () => ({
    contents: [{
