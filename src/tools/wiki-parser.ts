...
import { WikiParser, ParseOptions, StructureOptions } from '../parsers/wiki-content-parser.js';
import {
  validateRepo,
  validateAction,
  validateChapters,
  validateDepth,
  validateChapterDepths,
  validateSaveToFile
 ...
 */
export async function wikiParserTool(params: WikiParserToolParams): Promise<string> {
  ...
  if (action === 'structure') {
    ...
    if (params.chapters && !params.headers) {
      const headers: { [chapter: string]: string[] } = {};
 ...
 */
function parseChapterString(chapterString: string): { title: string; headers: string[] } {
