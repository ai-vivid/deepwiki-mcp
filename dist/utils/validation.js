...
export function validateRepo(repo) {
    ...
    if (parts.length !== 2) {
        throw new Error(`Invalid repository format. Expected "owner/repo", got "${repo}"`);
    }
    const [owner, repoName] = parts;
    if (!owner || !repoName) {
        throw new Error(`Invalid repository format. Both owner and repo name are required, got "${repo}"`);
    }
    ...
    if (!validPattern.test(owner)) {
        throw new Error(`Invalid repository owner "${owner}". Must contain only alphanumeric characters and hyphens, and cannot start or end with a hyphen.`);
    }
    if (!validPattern.test(repoName)) {
        throw new Error(`Invalid repository name "${repoName}". Must contain only alphanumeric characters and hyphens, and cannot start or end with a hyphen.`);
    }
}
/**
...
export function validateQuestion(question) {
    ...
    if (trimmed.length > 10000) {
        throw new Error(`Question is too long (${trimmed.length} characters). Maximum length is 10,000 characters.`);
    }
}
/**
 ...
 */
export function validateQueryId(queryId) {
    ...
    if (!validPattern.test(trimmed)) {
        throw new Error(`Invalid query ID format "${queryId}". Must contain only alphanumeric characters, hyphens, and underscores.`);
    }
}
/**
 ...
 */
export function validateChapterDepths(chapterDepths) {
 ...
 */
export function validateReferencesNumbers(referencesNumbers) {
    ...
    for (let i = 0; i < referencesNumbers.length; i++) {
        ...
        if (typeof refNum !== 'number') {
            throw new Error(`Reference number at index ${i} must be a number, got ${typeof refNum}`);
        }
        if (!Number.isInteger(refNum)) {
            throw new Error(`Reference number at index ${i} must be an integer, got ${refNum}`);
        }
        if (refNum < 1) {
            throw new Error(`Reference number at index ${i} must be positive, got ${refNum}`);
        }
    }
}
/**
 ...
 * @throws Error if array is invalid
 */
export function validateContextFiles(contextFiles) {
    ...
    for (let i = 0; i < contextFiles.length; i++) {
        ...
        if (typeof file !== 'string') {
            throw new Error(`Context file at index ${i} must be a string, got ${typeof file}`);
        }
 ...
 */
export function validateContextRanges(contextRanges) {
    ...
    for (const [file, range] of entries) {
        ...
        if (typeof range.start !== 'number' || !Number.isInteger(range.start)) {
            throw new Error(`Start line for file "${file}" must be an integer, got ${typeof range.start}`);
        }
        if (typeof range.end !== 'number' || !Number.isInteger(range.end)) {
            throw new Error(`End line for file "${file}" must be an integer, got ${typeof range.end}`);
        }
        if (range.start < 1) {
            throw new Error(`Start line for file "${file}" must be positive, got ${range.start}`);
        }
        if (range.end < range.start) {
            throw new Error(`End line (${range.end}) for file "${file}" must be greater than or equal to start line (${range.start})`);
        }
    }
}
/**
 ...
 */
export function validateSaveToFile(saveToFile) {
    ...
    if (normalized !== 'save-only' && normalized !== 'save-and-show') {
        throw new Error(`saveToFile must be either "save-only" or "save-and-show", got "${saveToFile}"`);
    }
}
/**
...
export function validateAction(action) {
    ...
    if (normalized !== 'structure' && normalized !== 'extract') {
        throw new Error(`Action must be either "structure" or "extract", got "${action}"`);
    }
}
