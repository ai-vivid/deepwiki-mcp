...
export class DeepwikiAutomator {
  ...
  private answerPage: Page | null = null;
  private static cachedExecutablePath: string | null = null;
   ...
   */
  async askQuestion(options: DeepwikiQuestionOptions): Promise<DeepwikiResponse> {
    const {
      repoUrl,
      question,
      useDeepResearch = false,
      headless = true,
      timeout = useDeepResearch ? 900000 : 300000  // Not used directly, but kept for compatibility
    } = options;
    ...
    try {
      ...
      await newPage.evaluate((queryId: string) => {
   ...
   */
  async goDeeper(options: DeepwikiGoDeeperOptions): Promise<DeepwikiResponse> {
    const {
      queryId,
      headless = true
    } = options;
   ...
   */
  async sendFollowUp(options: DeepwikiFollowUpOptions): Promise<DeepwikiResponse> {
    const {
      queryId,
      followUpQuestion,
      useDeepResearch = false,
    ...
    try {
      ...
      await this.page!.addInitScript((queryId: string) => {
        const storage = (globalThis as any).localStorage;
        storage.setItem('user_query_history', JSON.stringify([{
          "id": queryId,
          "timestamp": new Date().toISOString()
  ...
  private constructQueryUrl(queryId: string): string {
  ...
  private async initBrowser(headless: boolean): Promise<void> {
    ...
    let executablePath: string | undefined = DeepwikiAutomator.cachedExecutablePath || undefined;
  ...
  private extractQueryId(url: string): string | null {
  ...
  private async enableDeepResearch(): Promise<void> {
   ...
   */
  private parseCustomPollSchedule(envVar: string | undefined, defaultSchedule: number[]): number[] {
    if (!envVar || envVar.trim() === '') return defaultSchedule;
   ...
   */
  private async waitForNextScheduledCheck(startTime: number, scheduleIndex: number, pollSchedule: number[]): Promise<void> {
   ...
   */
  private async checkApiProgress(apiUrl: string): Promise<ApiResponse> {
  ...
  ): { newState: string; newLength: number; newChangeTime: number } {
  ...
  private async fetchApiResponse(queryId: string, useDeepResearch: boolean): Promise<Partial<DeepwikiResponse>> {
  ...
  private extractDataFromAllQueries(queries: ApiQuery[]): Partial<DeepwikiResponse> {
    let fullAnswer = '';
    const allReferences: DeepwikiResponse['references'] = [];
