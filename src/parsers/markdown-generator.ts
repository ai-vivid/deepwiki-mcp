// Markdown generator - based on the original json-to-markdown-parser.ts
...
export class MarkdownGenerator {
  private data: OutputResponse;

  constructor(data: OutputResponse) {
    this.data = data;
  }

  private formatQueryId(): string {
    return `# Query ID\n\n${this.data.query_id}\n`;
  }
  ...
  private formatRepoContextIds(): string {
  ...
  private getCodeFence(content: string): { open: string; close: string } {
  ...
  private formatReferencedFiles(includeAll: boolean, selectedNumbers?: number[]): string {
    ...
    const fileRangesMap = new Map<string, Set<string>>();
    ...
    for (const [fileName, entries] of Object.entries(this.data.referenced_files)) {
      if (!fileRangesMap.has(fileName)) continue;
      ...
      for (const entry of entries) {
        ...
        if (!includedRanges.has(entry.reference_range)) continue;
  ...
  private formatFullContext(
    ...
    fileRanges?: Map<string, { start: number; end: number }>
  ): string {
  ...
  private formatFullContextNamesOnly(): string {
    let output = '# Full Context Files\n\n';
  ...
  public generateMarkdown(options: MarkdownOptions): string {
    let markdown = '';
