export class WikiParser {
    ...
    extractHeaders(content, chapterTitle) {
        const headers = [];
    ...
    extractContent(options) {
        ...
        const chapterStructure = this.getChapterStructure();
        const requestedChapterMap = new Map();
        if (options.chapters) {
            for (const requestedChapter of options.chapters) {
                ...
                const matchingChapter = chapterStructure.find(ch => ch.title === requestedChapter ||
                    ch.id === requestedChapter ||
                    `${ch.id}: ${ch.title}` === requestedChapter ||
                    `Chapter ${ch.id}` === requestedChapter);
                if (matchingChapter) {
...
//# sourceMappingURL=wiki-content-parser.js.map